use concrete_commons::{BinaryKeyKind, Variance};
...
use concrete_core::crypto::{GlweSize, LweSize, PlaintextCount};
...
use concrete_core::math::polynomial::{PolynomialSize, Polynomial, MonomialDegree};
use concrete_core::math::tensor::{AsMutSlice, AsMutTensor, AsRefTensor, AsRefSlice};
...
unsafe fn fill_plaintext_list_with_expansion<T: UnsignedTorus>(
    ...
    polynomial.update_with_wrapping_monic_monomial_mul(MonomialDegree(-box_size/2));
    set_err!(err, ERR_NO_ERR);
}
